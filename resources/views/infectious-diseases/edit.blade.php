...
<div class="px-4 py-5 sm:px-6">
    <div class="flex justify-between">
        <h3 class="text-lg font-medium leading-6 text-gray-900">Edit Infectious Disease Record</h3>
        <div>
        ...
        <form action="{{ route('infectious-diseases.update', $infectiousDisease) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-3">
                    ...
                    <div class="mt-1">
                        <input type="number" name="total" id="total" min="0" value="{{ old('total', $infectiousDisease->total) }}" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                    @error('total')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-1">
                        <textarea id="notes" name="notes" rows="3" class="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm border-gray-300 rounded-md">{{ old('notes', $infectiousDisease->notes) }}</textarea>
                    </div>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
